...
export default function Inventory({
  ...
  const showTip = useCallback(
    (item, e) => setTip({ item, x: e.clientX, y: e.clientY }),
    [],
  );
  ...
  const onSlotLeave = useCallback(
    (id) => setHoverId((cur) => (cur === id ? null : cur)),
    [],
  );
  ...
  return (
    <aside className="inventory">
      <header className="inventory-header">
        <h2>Inventory</h2>
        <span className="inventory-gold" title="Gold">
          Gold: {gold}
        </span>
      ...
      <p className="inventory-hint">
        Click an item to grab it · click a slot to place it, or click the ground
        to drop it · <b>I</b> closes
      </p>
