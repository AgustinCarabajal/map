...
export const MODIFIERS = {
  // --- Ofensivos (armas) ---
  flat_phys: {
    stat: 'physicalDamage', mode: 'flat', label: '+# to physical damage',
    tiers: { low: [3, 12], mid: [15, 40], high: [50, 90] },
  },
  flat_fire: {
    stat: 'fireDamage', mode: 'flat', label: '+# to fire damage',
    tiers: { low: [3, 12], mid: [15, 40], high: [50, 90] },
  },
  flat_cold: {
    stat: 'coldDamage', mode: 'flat', label: '+# to cold damage',
    tiers: { low: [3, 12], mid: [15, 40], high: [50, 90] },
  },
  flat_light: {
    stat: 'lightningDamage', mode: 'flat', label: '+# to lightning damage',
    tiers: { low: [3, 12], mid: [15, 40], high: [50, 90] },
  },
  flat_sacred: {
    stat: 'sacredDamage', mode: 'flat', label: '+# to sacred damage',
    tiers: { low: [3, 12], mid: [15, 40], high: [50, 90] },
  },
  flat_demonic: {
    stat: 'demonicDamage', mode: 'flat', label: '+# to demonic damage',
    tiers: { low: [3, 12], mid: [15, 40], high: [50, 90] },
  },
  inc_atkspd: {
    stat: 'attackSpeed', mode: 'increased', label: '#% increased attack speed',
    tiers: { low: [3, 8], mid: [10, 18], high: [22, 35] },
  },
  inc_crit: {
    stat: 'critChance', mode: 'increased', label: '#% increased critical chance',
    tiers: { low: [5, 10], mid: [12, 25], high: [30, 50] },
  },
  inc_critdmg: {
    stat: 'critDamage', mode: 'increased', label: '#% increased critical damage',
    tiers: { low: [5, 15], mid: [20, 40], high: [50, 90] },
  },
  flat_proj: {
    stat: 'projectileCount', mode: 'flat', label: '+# to number of projectiles',
    tiers: { low: [1, 1], mid: [2, 3], high: [4, 5] },
  },

  // --- Defensivos (escudos / equipo) ---
  flat_def: {
    stat: 'defense', mode: 'flat', label: '+# to defense',
    tiers: { low: [8, 30], mid: [40, 120], high: [150, 350] },
  },
  inc_block: {
    stat: 'blockChance', mode: 'increased', label: '#% increased block chance',
    tiers: { low: [3, 8], mid: [10, 20], high: [25, 45] },
  },
  inc_dmgred: {
    stat: 'damageReduction', mode: 'increased', label: '#% increased damage reduction',
    tiers: { low: [2, 6], mid: [8, 14], high: [18, 30] },
  },

  // --- Generales (pueden ir en cualquier item) ---
  flat_hp: {
    stat: 'hp', mode: 'flat', label: '+# to life',
    tiers: { low: [10, 40], mid: [50, 120], high: [150, 300] },
  },
  flat_mp: {
    stat: 'mp', mode: 'flat', label: '+# to mana',
    tiers: { low: [5, 25], mid: [30, 80], high: [100, 200] },
  },
  flat_sacredShield: {
    stat: 'ss', mode: 'flat', label: '+# to sacred shield',
    tiers: { low: [5, 25], mid: [30, 80], high: [100, 200] },
  },
  flat_demonicShield: {
    stat: 'ds', mode: 'flat', label: '+# to demonic shield',
    tiers: { low: [5, 25], mid: [30, 80], high: [100, 200] },
  },
  inc_ms: {
    stat: 'movementSpeed', mode: 'increased', label: '#% increased movement speed',
    tiers: { low: [3, 8], mid: [10, 18], high: [22, 35] },
  },
}
...
export const SWORD_MODS = [
  'flat_phys', 'flat_fire', 'flat_cold', 'flat_light',
  'flat_sacred', 'flat_demonic',
  'inc_atkspd', 'inc_crit', 'inc_critdmg',
]
...
export const EQUIPMENT_MODS = ['flat_hp', 'flat_mp', 'flat_sacredShield', 'flat_demonicShield', 'inc_ms']
...
export function computeStats(baseStats, equipped = []) {
  ...
  for (const item of equipped) {
    ...
    if (b) {
      ...
      if (b.damage && dmgStat) stats[dmgStat] = (stats[dmgStat] || 0) + b.damage
      if (b.defense) stats.defense = (stats.defense || 0) + b.defense
      if (b.blockChance) stats.blockChance = (stats.blockChance || 0) + b.blockChance
    }
    ...
    for (const m of item.mods || []) {
      ...
      else pct[m.stat] = (pct[m.stat] || 0) + m.value
    }
  }
